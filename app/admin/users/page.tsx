...
interface ResetModalProps {
  user: UserItem;
  onClose: () => void;
  onConfirm: (uid: string) => Promise<string>;
}
...
function ResetModal({ user, onClose, onConfirm }: ResetModalProps) {
  ...
  async function handleReset() {
    setLoading(true);
    try {
      const pw = await onConfirm(user.uid);
      setTempPassword(pw);
    } catch (err: unknown) {
        ...
        <div>
          ...
          <p className="text-sm text-zinc-500 mt-1">
            {tempPassword
              ...
              : `Generate a temporary password for ${user.displayName ?? user.email}. They must change it on next login.`}
          </p>
        </div>
        ...
        {tempPassword ? (
          <div className="space-y-4">
            <div className="rounded-xl bg-white/[0.04] border border-white/[0.08] p-4 flex items-center justify-between gap-3">
              <code className="text-lg font-mono font-bold text-gold-400 tracking-widest">{tempPassword}</code>
              <button
            ...
            <div className="flex gap-3">
              <a
                href={`https://wa.me/2348020827133?text=Your%20temporary%20password%20for%20Shaka%20Saves%20is%3A%20${encodeURIComponent(tempPassword)}%0A%0APlease%20log%20in%20and%20change%20your%20password%20immediately.`}
                target="_blank"
...
export default function AdminUsersPage() {
  ...
  async function handleReset(uid: string): Promise<string> {
    ...
    await loadUsers();
    return json.data.temporaryPassword as string;
  }
              ...
              <div key={user.uid} className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-4 hover:bg-white/[0.02] transition-colors">
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    ...
                    {user.customerPhone && (
                      <p className="text-[11px] text-zinc-600">{user.customerPhone}</p>
                    )}
