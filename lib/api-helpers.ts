...
import { verifyRequestToken, unauthorizedResponse, ADMIN_UIDS } from "@/lib/auth";
...
export async function withRole(
  ...
  return withAuth(req, async (decoded) => {
    ...
    if (role === "admin" && !ADMIN_UIDS.has(decoded.uid)) return err("FORBIDDEN", "Insufficient permissions", 403);
    return handler(decoded);
  });
}
